...
verses.forEach(verse => verse.addEventListener('click', async event => {
    ...
    let loadVerse = await umurongo({targetValue, icyigisho, umwaka, igihembwe, title}).catch(() => `Please check your internet connection`);
    myModal.content = loadVerse.imirongo;
    myModal.title = loadVerse.soma || `Please check your internet connection`;
